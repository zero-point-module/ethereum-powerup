  ...
  return (
    <div className="w-full h-full relative">
      ...
      <div
        ...
        `}
        style={{
          boxShadow: state !== "off" ? "inset 0 0 10px rgba(0, 255, 0, 0.3)" : "none",
        }}
      >
        {/* No signal state */}
        {state === "off" && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-gray-600 font-mono text-xl tracking-widest">NO SIGNAL</div>
          </div>
        )}
          ...
          <div className="p-2 space-y-2 h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
            ...
            <div className="mb-3 flex space-x-2 items-center">
              ...
              <div className="flex-1 border border-[#004000] rounded bg-[#001200] p-1 flex items-center">
                <span className="text-[#00ff00] opacity-70 mr-1 text-xs">{">"}</span>
                <input
                  type="text"
                  placeholder="SEARCH..."
                  value={searchQuery}
              ...
              <button
                ...
                className={`
                  ...
                  ${state !== "active" ? "opacity-50 cursor-not-allowed" : "hover:bg-[#002800] cursor-pointer"}
                `}
                title={showOnlyInstalled ? "Show all modules" : "Show installed modules only"}
                disabled={state !== "active"}
              >
                <span className="text-[#00ff00] font-mono text-xs">{showOnlyInstalled ? "✓" : "⚙"}</span>
              </button>
            </div>
...
const ListItem = memo(function ListItem({ item, state, isSelected, isInstalled, onSelect }: ListItemProps) {
  return (
    <div
      className={`
        ...
        ${state === "active" ? "cursor-pointer hover:scale-[1.02]" : "cursor-not-allowed opacity-70"}
        ${
          isSelected
    ...
    >
      ...
      <span className={`text-[#00ff00] text-xs ml-2 w-8 text-right ${!isInstalled && "opacity-0"}`}>
        {isInstalled ? "✓" : ""}
      </span>
    </div>
  )
