...
export default function ScrambleText({ text, isScrambling, position, fontSize, maxWidth }: ScrambleTextProps) {
  ...
  useFrame(() => {
    if (isScrambling) {
      ...
      if (frameCount.current % 3 === 0) {
        let scrambled = ""
        const targetLength = Math.min(text.length, Math.floor(frameCount.current / 10))
